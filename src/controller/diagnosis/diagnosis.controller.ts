...
export class DiagnosisController {
    ...
    try {
      ...
      if (!authorizationHeader) {
        ...
        return res.status(HttpStatus.BAD_REQUEST).json(
          format_json(400, false, null, null, 'Authorization header is missing', null)
        );
      }
      ...
      if (getdata.status) {
        ...
        return res.status(HttpStatus.BAD_REQUEST).json(
          format_json(400, false, null, null, getdata.message, null)
        );
      }
    } catch (error) {
  ...
  async create(@Body() diagnosisDTO: DiagnosisDTO, @Res() res: Response, @Req() req: Request) {
    try {
      ...
      if (create.status) {
        ...
        return res.status(HttpStatus.BAD_REQUEST).json(
          format_json(400, false, null, null, create.message, null)
        );
      }
    } catch (error) {
      console.error('Server Error:', error);
  ...
  async update(@Param('id') id: number,@Body() diagnosisDTO: DiagnosisDTO, @Res() res: Response, @Req() req: Request) {
    try {
      ...
      const update = await this.diagnosisService.update(token, id, diagnosisDTO);
      ...
      if (update.status) {
        ...
        return res.status(HttpStatus.BAD_REQUEST).json(
          format_json(400, false, null, null, update.message, null)
        );
      }
    } catch (error) {
      console.error('Server Error:', error);
